...
- **OpenClaw 渠道**：通过 `openclaw sessions --json` 获取真实 token 数据（provider 返回），无需额外配置
- **API 代理**：启动后拦截发往大模型 API 的请求，自动记录 token 消耗；支持自定义上游地址（`--upstream`）
- **准确度**：真实 token（API 返回）/ 估算 token（tiktoken），数据库中标记了 `accuracy` 字段
